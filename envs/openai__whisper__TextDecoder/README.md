# `TextDecoder`  ·  RL kernel-engineering env

> Custom nn.Module 'TextDecoder' with non-trivial body.

**Source:** [`openai/whisper`](https://github.com/openai/whisper) · `whisper/model.py`
(lines 207–249)
**Novelty:** 0.10
**Tags:** `looks-generic`, `self-contained`, `uses-buffers`
**Selection mode:** `heuristic`

## Goal

Write an implementation of `TextDecoder` in `solution.py` that is
...
```python
TextDecoder.__init__(n_vocab, n_ctx, n_state, n_head, n_layer)
TextDecoder.forward(x, xa, kv_cache=None)
```
