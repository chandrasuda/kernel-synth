...
Goal
----
Write **Triton** kernels (no raw CUDA) that match ``reference.AutoregressiveWrapper``
...
def build(**kwargs):
    ...
    Must accept the same kwargs as ``reference.AutoregressiveWrapper`` and
    return something callable like ``module(*args, **kwargs)`` that
    produces the same shape/dtype/values within tolerance.
    """
    return reference.AutoregressiveWrapper(**kwargs)
