"""Benchmark for the Mamba2Simple kernel-engineering env.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        torch.manual_seed(0)
        eager_mod = reference.Mamba2Simple(**kwargs).to(DEVICE, DTYPE)
        eager_mod.eval()
    except Exception as e:
    ...
    if not args.check_only:
        try:
            torch.manual_seed(0)
            compile_target = reference.Mamba2Simple(**kwargs).to(DEVICE, DTYPE).eval()
