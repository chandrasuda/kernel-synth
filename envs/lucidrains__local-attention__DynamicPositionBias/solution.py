...
Goal
----
Write **Triton** kernels (no raw CUDA) that match ``reference.DynamicPositionBias``
...
def build(**kwargs):
    ...
    Must accept the same kwargs as ``reference.DynamicPositionBias`` and
    return something callable like ``module(*args, **kwargs)`` that
    produces the same shape/dtype/values within tolerance.
    """
    return reference.DynamicPositionBias(**kwargs)
