# `DynamicPositionBias`  ·  RL kernel-engineering env
...
Write an implementation of `DynamicPositionBias` in `solution.py` that is
...
```python
DynamicPositionBias.__init__(dim, heads)
DynamicPositionBias.forward(i, j)
```
