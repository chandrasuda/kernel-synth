# `NaViT`  ·  RL kernel-engineering env

> forward uses topk; file imports einops, einops, einops, einops; non-trivial parameter init

**Source:** [`lucidrains/vit-pytorch`](https://github.com/lucidrains/vit-pytorch) · `vit_pytorch/na_vit_nested_tensor_3d.py`
(lines 135–323)
**Novelty:** 0.88
...
Write an implementation of `NaViT` in `solution.py` that is
...
```python
NaViT.__init__(image_size, max_frames, patch_size, frame_patch_size, num_classes, dim, depth, heads, mlp_dim, channels=3, dim_head=64, dropout=0.0, emb_dropout=0.0, num_registers=4, qk_rmsnorm=True, token_dropout_prob=None)
NaViT.forward(volumes)
```
