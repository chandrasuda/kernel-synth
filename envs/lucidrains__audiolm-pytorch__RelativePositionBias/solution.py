...
Goal
----
Write **Triton** kernels (no raw CUDA) that match ``reference.RelativePositionBias``
...
def build(**kwargs):
    ...
    Must accept the same kwargs as ``reference.RelativePositionBias`` and
    return something callable like ``module(*args, **kwargs)`` that
    produces the same shape/dtype/values within tolerance.
    """
    return reference.RelativePositionBias(**kwargs)
