# `MultiScaleDiscriminator`  ·  RL kernel-engineering env
...
Write an implementation of `MultiScaleDiscriminator` in `solution.py` that is
...
```python
MultiScaleDiscriminator.__init__(channels=16, layers=4, groups=(4, 16, 64, 256), chan_max=1024, input_channels=1)
MultiScaleDiscriminator.forward(x, return_intermediates=False)
```
