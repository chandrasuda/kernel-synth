...
Goal
----
Write **Triton** kernels (no raw CUDA) that match ``reference.MultiScaleDiscriminator``
...
def build(**kwargs):
    ...
    Must accept the same kwargs as ``reference.MultiScaleDiscriminator`` and
    return something callable like ``module(*args, **kwargs)`` that
    produces the same shape/dtype/values within tolerance.
    """
    return reference.MultiScaleDiscriminator(**kwargs)
