...
def build_module_kwargs() -> dict:
    ...
    return dict(
        ...
        dim=64,
        depth=2,
        num_semantic_tokens=None  # TODO: pick a value for 'num_semantic_tokens',
    )


def build_forward_inputs() -> tuple[tuple, dict]:
    """Return (positional args, keyword args) for the forward call."""
    semantic_token_ids = torch.randn(BATCH, SEQ_LEN, HIDDEN)  # TODO: shape for 'semantic_token_ids'
    coarse_token_ids = torch.randn(BATCH, SEQ_LEN, HIDDEN)  # TODO: shape for 'coarse_token_ids'
    return (semantic_token_ids, coarse_token_ids,), {}
